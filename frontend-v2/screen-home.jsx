...
const ScreenHome = ({ onNavigate, tweaks }) => {
  ...
  return (
    <div className="screen">
      <Hero onNavigate={onNavigate} progress={overallProgress} layout={tweaks.hero_layout} />

      <section className="container" style={{ paddingTop: 32 }}>
        <ContinueCard course={current} chapter={currentChapter} onNavigate={onNavigate} />
      </section>
      ...
      <section className="container" style={{ paddingTop: 56 }}>
        <div className="section-h">
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => onNavigate("courses")}>
            查看全部 <Icon name="arrowRight" size={12} />
          </button>
        </div>
        <LearningPath onNavigate={onNavigate} />
      </section>
      ...
      <section className="container" style={{ paddingTop: 56, paddingBottom: 80 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
          {COURSES.slice(2, 6).map(c => (
            <CourseCard key={c.id} course={c} onOpen={() => onNavigate("course", { id: c.id })} />
          ))}
        </div>
      </section>
    </div>
...
const ContinueCard = ({ course, chapter, onNavigate }) => {
  ...
  return (
    <div className="card" style={{
      ...
      <div className="vstack" style={{ gap: 8 }}>
        <button className="btn btn-primary btn-lg" onClick={() => onNavigate("chapter", { id: chapter.id })}>
          <Icon name="play" size={13} /> 继续阅读
        </button>
        <button className="btn btn-ghost btn-sm" onClick={() => onNavigate("course", { id: course.id })}>
          查看大纲
        </button>
      </div>
    </div>
