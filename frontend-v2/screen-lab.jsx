...
const ScreenLab = ({ params, onNavigate }) => {
  const lab = SAMPLE_LAB;
  const [code, setCode] = React.useState(lab.starter);
  const [tab, setTab] = React.useState("output"); // output | tests | console
  const [running, setRunning] = React.useState(false);
  const [results, setResults] = React.useState(lab.tests);
      ...
      <div style={{
        ...
        <div className="container hstack" style={{ height: 48, gap: 12 }}>
          <nav className="crumb" style={{ flex: 1, minWidth: 0 }}>
            ...
            <a href="#" onClick={(e) => { e.preventDefault(); onNavigate("course", { id: 3 }); }}>{lab.course_title}</a>
            <Icon name="chevronRight" size={11} className="sep" />
...
Object.assign(window, { ScreenLab });
