...
const Topbar = ({ route, onNavigate, onOpenSearch }) => {
  const tabs = [
        ...
        <nav className="nav" aria-label="主导航">
          {tabs.map(t => (
            <a
              ...
              onClick={(e) => { e.preventDefault(); onNavigate(t.id === "discuss" ? "discuss" : t.id); }}
            >{t.label}</a>
          ))}
        </nav>
...
const MobileNav = ({ route, onNavigate }) => {
  const tabs = [
    { id: "home", label: "首页", icon: "home" },
    { id: "courses", label: "课程", icon: "book" },
    { id: "progress", label: "进度", icon: "trending" },
  ...
  const active = route.name === "course" || route.name === "chapter" || route.name === "lab" ? "courses" : route.name;
  return (
    <nav className="mobile-nav" aria-label="底部导航">
      {tabs.map(t => (
        <a key={t.id} href="#" className={active === t.id ? "active" : ""} onClick={(e) => { e.preventDefault(); onNavigate(t.id); }}>
              ...
              <Group title="课程">
                {courseHits.map(c => (
                  <Row key={c.id} icon="book" title={c.title} sub={c.subtitle}
                       onClick={() => { onClose(); onNavigate("course", { id: c.id }); }} />
                ))}
              </Group>
              <Group title="章节 / 实验">
                {chapterHits.map(ch => (
                  <Row key={ch.id} icon={ch.type === "lab" ? "flask" : "fileText"}
                       ...
                       onClick={() => {
                         onClose();
                         if (ch.type === "lab") onNavigate("lab", { id: ch.id });
                         else onNavigate("chapter", { id: ch.id });
                       }} />
                ))}
              </Group>
              <Group title="操作">
                <Row icon="trending" title="查看我的进度" onClick={() => { onClose(); onNavigate("progress"); }} />
...
Object.assign(window, { Topbar, MobileNav, CommandPalette });
