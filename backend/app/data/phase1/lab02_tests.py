...
pipeline = ETLPipeline()  # noqa: F821
...
assert (
    report["null_counts"]["email"] == 3
), f"email缺失应为3, 得到{report['null_counts'].get('email', 0)}"
...
cleaned = pipeline.clean(
    customers, fillna={"email": "unknown@test.com"}, remove_outliers={"age": (0, 120)}
)
