...
def test_split_data():
    ...
    assert (
        abs(train_pos_rate - test_pos_rate) < 0.1
...
def test_identify_column_types():
    ...
    from lab01_starter import identify_column_types, load_and_prepare_data
    ...
    # No overlap
    assert not set(numeric_features) & set(
...
def test_build_preprocessor():
    ...
    from lab01_starter import build_preprocessor, identify_column_types, load_and_prepare_data
...
def test_feature_engineering():
    ...
    from lab01_starter import engineer_features, load_and_prepare_data
    ...
    assert len(X_eng.columns) >= len(X.columns), "Feature engineering should add columns"

    # Check specific new features
    if "charges_per_month" in X_eng.columns:
        assert not X_eng["charges_per_month"].isnull().all(), "charges_per_month should have values"
