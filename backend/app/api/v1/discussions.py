...
from app.models import Comment, Course, Discussion, User
...
def get_discussion(
    ...
    if not discussion:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="讨论不存在")

    # 获取评论（只获取顶层评论，不嵌套回复）
    comments = (
        db.query(Comment)
        .filter(Comment.discussion_id == discussion_id, Comment.parent_id.is_(None))
        .order_by(Comment.is_solution.desc(), Comment.created_at.desc())
        .all()
    )
...
def update_discussion(
    ...
    if discussion.user_id != current_user.id and current_user.role not in ["admin", "teacher"]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="没有权限修改此讨论")
...
def delete_discussion(
    ...
    # 检查权限（只有作者或管理员可以删除）
    if discussion.user_id != current_user.id and current_user.role not in ["admin", "teacher"]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="没有权限删除此讨论")
...
def create_comment(
    ...
    if discussion.is_locked:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="该讨论已被锁定，无法回复"
        )
...
def like_discussion(
    ...
    existing = (
        ...
        .filter(
            DiscussionLike.user_id == current_user.id, DiscussionLike.discussion_id == discussion_id
        )
        .first()
    )
    ...
    if existing:
        ...
        # Like
        db_like = DiscussionLike(user_id=current_user.id, discussion_id=discussion_id)
        db.add(db_like)
